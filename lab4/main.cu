...
int main (int argc, char *argv[])
{
    ...
    cudaHostAlloc((void**)&A_h ,sizeof(float)*A_sz, cudaHostAllocDefault);
    for (unsigned int i=0; i < A_sz; i++) { A_h[i] = (rand()%100)/100.00; }

    //B_h = (float*) malloc( sizeof(float)*B_sz );
    cudaHostAlloc((void**)&B_h ,sizeof(float)*B_sz, cudaHostAllocDefault);
    for (unsigned int i=0; i < B_sz; i++) { B_h[i] = (rand()%100)/100.00; }

    //C_h = (float*) malloc( sizeof(float)*C_sz );
    cudaHostAlloc((void**)&C_h ,sizeof(float)*C_sz, cudaHostAllocDefault);
    ...
    startTime(&timer);
    streamVecAdd(A_h, B_h, C_h, VecSize); //In kernel.cu
